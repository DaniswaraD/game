## v56.1 — Full package QA
- Unified responsive menu sizing: fluid parent-driven layout with bounded fixed touch targets.
- Main menu, profile, Global Champion, title and 9 action buttons use the same light/pastel visual language as the other screens.
- Removed two unused legacy text artifacts from the release package.
- Embedded CSS parsed with zero parser errors; JavaScript passed syntax checks; HTML IDs are unique; local asset references resolve.
- Service-worker cache is versioned as v56.1-light.

# Danis.Shooter v56.1 — Responsive UI & Full QA Build

v56.1 keeps the existing gameplay systems intact while applying the unified responsive UI system and a full package QA pass.
...
## v50.9 — Gameplay clarity + low-end pass
- Removed the large opaque HUD containers that could cover the combat arena.
- Reworked gameplay HUD into compact edge chips so the center remains visible.
- Reduced pause, pet, skill, kill-feed and global-stat UI footprint.
- Dialogue is now a compact bottom strip instead of a large screen-blocking panel.
- Reduced the 9 main-menu navigation tiles so they read as compact navigation rather than oversized cards.
- Removed expensive backdrop blur from frequently visible gameplay controls.
- Kept the shop as the only intentional scrolling surface.
- Service-worker cache bumped to `v50.9-light`.
- JavaScript syntax and HTML/ID integrity checks passed.
