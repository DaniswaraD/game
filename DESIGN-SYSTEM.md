# Danis.Shooter v50 — Visual Direction
...
## V50 — Mega Visual/UX Reset
- Replaced the accumulated v12–v42 visual layering with a final light sci-fi arcade material system.
- Main menu is a composition: brand + settings, player identity, Global #1 information, primary mission CTA, then a compact utility rail.
- Utility actions are intentionally quieter than Start Mission and no longer compete as identical center cards.
- Typography uses Space Grotesk for display/labels and DM Sans for UI/body.
- Light surfaces, cool steel borders, cyan system feedback, amber progression and coral danger form one consistent palette.
- Non-shop screens do not scroll; shop remains the intentional scroll surface.
- Responsive layout is defined for desktop, tablet, narrow mobile and short-height displays.
- Visual QA includes JavaScript syntax checks and structural checks for style placement, scroll contracts and menu grid geometry.


## V50 QA Patch
- Fixed main-menu CSS grid collision: the primary CTA uses `grid-area: cta`, while the utility tile rail uses `grid-area: actions`.
- Fixed the mobile Global Champion selector so its compact padding applies to `.menu-global-profile`.
- Confirmed all 312 HTML IDs are unique.
- Confirmed JavaScript syntax for `game.js`, `multiplayer.js`, `addings.js`, `library-loader.js`, and `sw.js`.
- Main menu scroll contract remains locked: page/screen does not scroll; Shop is the intentional scroll surface.
