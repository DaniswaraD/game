...
var shopPreviewCtx=null,shopPreviewCanvas=null,shopPreviewAnim=null,shopPreviewFiring=0,shopPreviewFocus=null;
var shipFxTimer=0;
var mpBadgeXpText='';
var winShown=false;
...
function checkFinalBossCompletion(){
  if(!currentLevel||!currentLevel.isFinal||winShown)return false;
  var total=currentLevel.finalBossCount||BOSS_TYPES.length;
  if(finalBossNext>=total&&bosses.length===0){
    bossWrap.classList.remove('on');
  ...
  for(var bi=bosses.length-1;bi>=0;bi--){
    ...
    if(boss.hp<=0){
      if(isNetworkGame&&window.DS_MP.sharedCombat&&boss.id){
        ...
        if(window.MP_damageGlobalBoss)window.MP_damageGlobalBoss(boss.id,999999);
        bosses.splice(bi,1);
        /* In shared combat the host owns final-boss progression. The old branch
           removed the network boss but never advanced finalBossNext, so Chapter 5
           could get stuck after the last boss. */
        if(bosses.length===0&&currentLevel.isFinal&&!winShown&&amHost()){
          finalBossNext++;
          if(!checkFinalBossCompletion()){
            setTimeout(function(){
              if(!winShown&&bosses.length===0&&amHost())spawnBossForFinal(finalBossNext);
            },220);
          }
        }
      }else{
        killBoss(boss);
        bosses.splice(bi,1);
        if(bosses.length===0&&currentLevel.isFinal&&!winShown){
          finalBossNext++;
          if(!checkFinalBossCompletion()){
            setTimeout(function(){if(!winShown&&bosses.length===0&&(!isNetworkGame||!window.DS_MP.sharedCombat||amHost()))spawnBossForFinal(finalBossNext);},220);
          }
        }
      }
    }
...
var dom={
  ...
  shopPreview:document.getElementById('shopPreview'),shopPreviewName:document.getElementById('shopPreviewName'),shopPreviewStats:document.getElementById('shopPreviewStats'),shopPreviewLevel:document.getElementById('shopPreviewLevel'),skillTestBtn:document.getElementById('skillTestBtn'),
...
function updateShopPreviewInfo(){
  var nameEl=dom.shopPreviewName,statsEl=dom.shopPreviewStats,lvEl=dom.shopPreviewLevel;
  if(!nameEl||!statsEl||!lvEl)return;
  var type=(shopPreviewFocus&&shopPreviewFocus.type)||shopTab,item=null,lv=0,stats='';
  var focusId=(shopPreviewFocus&&shopPreviewFocus.id)||null;
  if(type==='gun'&&focusId)item=findGun(focusId)||null;
  else if(type==='ship'&&focusId)item=findShip(focusId)||null;
  else if(type==='shape'&&focusId)item=findShape(focusId)||null;
  else if(type==='skill'&&focusId)item=findSkill(focusId)||null;
  else if(type==='pet'&&focusId)item=findPet(focusId)||null;
  if(type==='gun'){item=item||findGun(save.selectedGun)||GUNS[0];lv=getUpgradeLevel('gun',item.id);var gb=getGunUpgradeBonus(item.id);stats='DMG +'+Math.round((gb.dmgMult-1)*100)+'%  •  RoF +'+Math.round((gb.rofMult-1)*100)+'%  •  SPD +'+Math.round((gb.speedMult-1)*100)+'%';}
  else if(type==='ship'){item=item||findShip(save.selectedShip)||SHIPS[0];lv=getUpgradeLevel('ship',item.id);var hp=(item.passive&&item.passive.hpMult)?'HP +'+Math.round((item.passive.hpMult-1)*100)+'%  •  ':'';stats=hp+'UPGRADE HP +'+(lv*10)+'  •  FX ×'+(1+lv*.18).toFixed(2);}
  else if(type==='shape'){item=item||findShape(save.selectedShape)||PLAYER_SHAPES[0];lv=0;stats=(item.passiveDesc||'FRAME ACTIVE')+'  •  VISUAL FRAME';}
  else if(type==='skill'){item=item||findSkill(save.selectedSkill)||SKILLS[0];lv=getUpgradeLevel('skill',item.id);stats='CD -'+Math.round(lv*8)+'%  •  DURASI +'+Math.round(lv*12)+'%  •  POWER FX ×'+(1+lv*.16).toFixed(2);}
  else if(type==='pet'){item=item||findPet(save.selectedPet)||PETS[0];lv=getUpgradeLevel('pet',item.id);stats='UPGRADE LV '+lv+'  •  EFFECT POWER ×'+(1+lv*.20).toFixed(2);}
  else {item=findStart(focusId)||STARTING_UPGRADES[0];stats=(item?item.desc:'SYSTEM BONUS ACTIVE');}
  nameEl.textContent=String(item&&item.name||'LOADOUT').toUpperCase();
  statsEl.textContent=stats;
  lvEl.textContent=lv?'LV '+lv+'/5':'ACTIVE';
}

function shopPreviewColor(id){
  var palette=['#53d7e8','#4aa9ff','#9b8cff','#67e8b7','#ffbf4b','#ff77a9','#b9f4ff'];
  var h=0,st=String(id||'item');
  for(var i=0;i<st.length;i++)h=(h*31+st.charCodeAt(i))>>>0;
  return palette[h%palette.length];
}
function drawShopPreviewItem(g,cx,cy,focusType,item,lv,accent,cw,ch,phase){
  var core=Math.min(cw,ch)*.16, scale=1+lv*.045;
  g.save();
  /* Every item gets its own geometry; upgrade levels add a new visible module/ring. */
  g.translate(cx,cy); g.scale(scale,scale);
  g.shadowBlur=18+lv*4; g.shadowColor=accent;
  if(focusType==='gun'){
    var gid=item&&item.id||'bullet';
    var barrel=gid==='rocket'||gid==='nova'||gid==='scatterbomb'?'#ffbf4b':gid==='laser'||gid==='deathray'||gid==='railgun'?'#9ff6ff':gid==='poison'?'#7ee06f':gid==='plasma'||gid==='vortex'?'#b88cff':accent;
    var w=gid==='shotgun'||gid==='spread'||gid==='triple'||gid==='burst'?46:30;
    var h=gid==='plasma'||gid==='nova'?26:gid==='rocket'||gid==='drill'?34:18;
    g.fillStyle='#f6fcff';g.strokeStyle=barrel;g.lineWidth=2;
    if(gid==='laser'||gid==='deathray'||gid==='railgun'){
      g.fillStyle=barrel;g.fillRect(-4,-48,8,74);g.strokeRect(-7,-48,14,74);
      g.fillStyle='#fff';g.fillRect(-2,-40,4,58);
    }else if(gid==='zigzag'||gid==='boomerang'){
      g.beginPath();g.moveTo(-w/2,10);g.lineTo(-w/5,-12);g.lineTo(0,8);g.lineTo(w/5,-12);g.lineTo(w/2,10);g.stroke();g.fillStyle=barrel;g.fill();
    }else if(gid==='rocket'||gid==='nova'||gid==='scatterbomb'){
      g.beginPath();g.moveTo(0,-h/2-10);g.lineTo(w/2,-h/2+7);g.lineTo(w/3,h/2);g.lineTo(-w/3,h/2);g.lineTo(-w/2,-h/2+7);g.closePath();g.fill();g.stroke();
      g.fillStyle='#fff';g.beginPath();g.arc(0,0,5+lv,0,Math.PI*2);g.fill();
    }else if(gid==='homing'||gid==='lightning'||gid==='vortex'){
      g.fillStyle=barrel;g.beginPath();g.arc(0,0,core*.55,0,Math.PI*2);g.fill();
      for(var hi=0;hi<3+lv;hi++){var ha=phase*(1+hi*.08)+hi*2.094;g.beginPath();g.arc(Math.cos(ha)*28,Math.sin(ha)*20,3,0,Math.PI*2);g.fill();}
    }else{
      g.fillStyle=barrel;g.fillRect(-w/2,-h/2,w,h);g.strokeRect(-w/2,-h/2,w,h);
      g.fillStyle='#fff';g.beginPath();g.arc(0,0,Math.max(3,4+lv*.5),0,Math.PI*2);g.fill();
    }
  }else if(focusType==='skill'){
    var sid=item&&item.id||'skill';
    g.strokeStyle=accent;g.fillStyle=accent;g.lineWidth=2.5;
    if(sid==='health'||sid==='phoenix'){
      g.fillRect(-8,-28,16,56);g.fillRect(-28,-8,56,16);
    }else if(sid==='spikes'||sid==='wipeout'){
      for(var si=0;si<8;si++){var sa=si*Math.PI/4;g.beginPath();g.moveTo(Math.cos(sa)*10,Math.sin(sa)*10);g.lineTo(Math.cos(sa)*42,Math.sin(sa)*42);g.stroke();}
      g.beginPath();g.arc(0,0,18,0,Math.PI*2);g.stroke();
    }else if(sid==='chain'||sid==='lightning'){
      g.beginPath();g.moveTo(-25,-35);g.lineTo(-5,-7);g.lineTo(-17,-7);g.lineTo(8,36);g.lineTo(4,8);g.lineTo(20,8);g.stroke();
    }else if(sid==='voidbeam'||sid==='overdrive'){
      g.fillRect(-5,-58,10,116);g.globalAlpha=.35;g.fillRect(-12,-58,24,116);g.globalAlpha=1;g.strokeRect(-22,-22,44,44);
    }else{
      g.beginPath();g.arc(0,0,30+lv*5,phase*.7,phase*.7+Math.PI*1.55);g.stroke();
      g.beginPath();g.arc(0,0,14+lv*2,0,Math.PI*2);g.stroke();
    }
  }else if(focusType==='pet'){
    var ps=petSpriteCache[item&&item.id];
    if(ps&&ps.canvas){var pc=ps;var sc=Math.min(1.25,cw/pc.width*.30);g.shadowBlur=14;g.drawImage(pc.canvas,-pc.canvas.width*sc/2,-pc.canvas.height*sc/2,pc.canvas.width*sc,pc.canvas.height*sc);}
    else {g.fillStyle=accent;g.beginPath();g.arc(0,0,26+lv*4,0,Math.PI*2);g.fill();g.fillStyle='#fff';g.beginPath();g.arc(-8,-5,4,0,Math.PI*2);g.arc(8,-5,4,0,Math.PI*2);g.fill();}
    for(var pi=0;pi<4+lv;pi++){var pa=phase+pi*Math.PI*2/(4+lv);g.strokeStyle=accent;g.beginPath();g.arc(Math.cos(pa)*45,Math.sin(pa)*30,3,0,Math.PI*2);g.stroke();}
  }else if(focusType==='start'){
    var sid2=item&&item.id||'system';
    g.strokeStyle=accent;g.lineWidth=2;
    if(sid2==='hull'){
      g.strokeRect(-42,-30,84,60);g.strokeRect(-30,-20,60,40);g.fillStyle='#fff';g.fillRect(-28,-4,56,8);
    }else if(sid2==='power'){
      g.beginPath();g.moveTo(-8,-44);g.lineTo(16,-8);g.lineTo(2,-8);g.lineTo(12,44);g.lineTo(-16,4);g.lineTo(-2,4);g.closePath();g.fillStyle=accent;g.fill();g.stroke();
    }else if(sid2==='rapid'){
      for(var rb=0;rb<3+lv;rb++){var ry=-28+rb*14;g.strokeRect(-42,ry,84,7);g.fillStyle=rb%2?'#fff':accent;g.fillRect(-36,ry+2,18+lv*3,3);}
    }else if(sid2==='haste'){
      g.beginPath();g.arc(0,0,34,0,Math.PI*2);g.stroke();g.beginPath();g.moveTo(0,0);g.lineTo(0,-24);g.moveTo(0,0);g.lineTo(19,10);g.stroke();
    }else if(sid2==='luck'){
      g.beginPath();for(var ls=0;ls<10;ls++){var la=-Math.PI/2+ls*Math.PI/5,lr=ls%2?18:40;var lx=Math.cos(la)*lr,ly=Math.sin(la)*lr;ls===0?g.moveTo(lx,ly):g.lineTo(lx,ly);}g.closePath();g.fillStyle=accent;g.fill();g.stroke();
    }else if(sid2==='regen'){
      g.beginPath();g.arc(0,0,35,0,Math.PI*2);g.stroke();g.fillStyle=accent;g.fillRect(-7,-25,14,50);g.fillRect(-25,-7,50,14);
    }else if(sid2==='crit'){
      g.beginPath();g.moveTo(-38,18);g.lineTo(-8,-38);g.lineTo(5,-8);g.lineTo(38,-18);g.stroke();g.fillStyle='#fff';g.fillRect(12,-5,10,10);
    }else{
      g.strokeRect(-34,-34,68,68);g.strokeRect(-20,-20,40,40);g.fillStyle='#fff';g.fillRect(-5,-5,10,10);
    }
  }
  g.restore();
  /* Upgrade hardware modules: LV1..LV5 never look identical. */
  if(lv>0){
    g.save();g.strokeStyle=accent;g.lineWidth=1+lv*.35;g.globalAlpha=.5+.08*lv;
    var rr=48+lv*9;g.beginPath();g.arc(cx,cy,rr,phase*.25,phase*.25+Math.PI*(.65+.22*lv));g.stroke();
    g.fillStyle=accent;
    for(var m=0;m<lv;m++){var ma=phase*.35+m*6.283/Math.max(1,lv);g.fillRect(cx+Math.cos(ma)*rr-2.5,cy+Math.sin(ma)*rr-2.5,5,5);}
    /* Each upgrade level gets a distinct hardware language, not only a larger ring. */
    var moduleCount=Math.min(6,lv+1),itemKey=String(item&&item.id||focusType||'item'),seed=0;
    for(var sk=0;sk<itemKey.length;sk++)seed=(seed*33+itemKey.charCodeAt(sk))>>>0;
    for(var mm=0;mm<moduleCount;mm++){
      var aa=phase*.22+(mm+1)*6.283/moduleCount+(seed%17)*.03;
      var mx=cx+Math.cos(aa)*(rr+8),my=cy+Math.sin(aa)*(rr+8);
      g.fillRect(mx-3,my-3,6,6);
      if((seed+mm)%3===0){g.beginPath();g.arc(mx,my,8+lv*1.5,aa-.45,aa+.45);g.stroke();}
    }
    g.restore();
  }
}
...
function renderShopPreview(dt){
  ...
  var focusType=(shopPreviewFocus&&shopPreviewFocus.type)||shopTab,focusId=(shopPreviewFocus&&shopPreviewFocus.id)||null;
  if(focusType==='ship'&&focusId)ship=findShip(focusId)||ship;
  if(focusType==='gun'&&focusId)gun=findGun(focusId)||gun;
  if(focusType==='shape'&&focusId)shape=findShape(focusId)||shape;
  var focusSkill=(focusType==='skill'&&focusId)?findSkill(focusId):null;
  var focusPet=(focusType==='pet'&&focusId)?findPet(focusId):null;
  var focusStart=(focusType==='start'&&focusId)?findStart(focusId):null;
  var shipLv=getUpgradeLevel('ship',ship&&ship.id),gunLv=getUpgradeLevel('gun',gun&&gun.id),skillLv=getUpgradeLevel('skill',focusSkill&&focusSkill.id),petLv=getUpgradeLevel('pet',focusPet&&focusPet.id);
  /* Preview upgrades must belong to the focused item. The previous pass used
     Math.max(...) across every category, so a LV5 ship could make an LV1 gun
     preview look upgraded even when the gun itself was untouched. */
  var previewLv=0;
  if(focusType==='gun')previewLv=gunLv;
  else if(focusType==='ship'||focusType==='shape')previewLv=shipLv;
  else if(focusType==='skill')previewLv=skillLv;
  else if(focusType==='pet')previewLv=petLv;
  else if(focusType==='start'&&focusId)previewLv=getUpgradeLevel('start',focusId);
  ...
  for(var rr=34;rr<Math.min(cw,ch)*(.46+Math.min(.10,Math.max(shipLv,gunLv)*.02));rr+=28){g.strokeStyle=hexToRgba(accent,.09);g.beginPath();g.arc(cx,cy,rr,phase*.15+(rr%56)/80,phase*.15+(rr%56)/80+Math.PI*1.55);g.stroke();}
  // Orbiting telemetry particles, capped for low-end devices.
  var count=Math.min(34,(PERF_MODE?14:22)+Math.max(shipLv,gunLv,skillLv,petLv)*2);
  for(var i=0;i<count;i++){
  ...
  if(glow){g.save();g.globalAlpha=.22+.035*Math.max(shipLv,gunLv,skillLv,petLv);g.drawImage(glow.normal.canvas,cx-glow.R*.7,cy-glow.R*.7,glow.R*1.4,glow.R*1.4);g.restore();}
  if(focusType==='ship'||focusType==='shape'){
    ...
    drawShopPreviewItem(g,cx,cy,focusType,focusType==='gun'?gun:focusType==='skill'?focusSkill:focusType==='pet'?focusPet:focusType==='start'?focusStart:STARTING_UPGRADES[0],previewLv,shopPreviewColor(focusId||focusType),cw,ch,phase);
  }
  // Weapon test lane.
  /* Only the focused weapon gets the firing demonstration. Other shop tabs
     should preview their own system rather than silently showing the equipped gun. */
  shopPreviewFiring+=dt;
  if(focusType==='gun'&&shopPreviewFiring>=.16){shopPreviewFiring=0;var previewGunId=gun?gun.id:save.selectedGun;var ps=playerProjSpriteCache[previewGunId]||playerProjSpriteCache.bullet;shopPreviewAnim=shopPreviewAnim||[];shopPreviewAnim.push({x:cx+(Math.sin(phase*1.8)*3),y:cy-28,vy:-320,life:.72,maxLife:.72,sprite:ps});if(shopPreviewAnim.length>18)shopPreviewAnim.shift();}
  if(shopPreviewAnim){for(var j=shopPreviewAnim.length-1;j>=0;j--){var q=shopPreviewAnim[j];q.life-=dt;q.y+=q.vy*dt;if(q.life<=0){shopPreviewAnim.splice(j,1);continue;}var qa=q.life/q.maxLife;g.save();g.globalAlpha=qa;g.drawImage(q.sprite.canvas,q.x-q.sprite.cx*(.52+.035*gunLv),q.y-q.sprite.cy*(.52+.035*gunLv),q.sprite.canvas.width*(.52+.035*gunLv),q.sprite.canvas.height*(.52+.035*gunLv));g.restore();}}
  /* Skill upgrades get their own live effect instead of only changing a number. */
  if(focusSkill){
    var skillAccent=focusSkill.color||accent, skillRadius=28+skillLv*7;
    g.save();g.globalAlpha=.16+.035*skillLv;g.strokeStyle=skillAccent;g.lineWidth=1.4+skillLv*.25;
    if(focusSkill.id==='voidbeam'||focusSkill.id==='laser'){g.beginPath();g.moveTo(cx,cy-45);g.lineTo(cx,cy-45-skillRadius*1.7);g.stroke();}
    else if(focusSkill.id==='chain'){for(var ci=0;ci<3+skillLv;ci++){var ca=phase*1.4+ci*6.283/(3+skillLv);g.beginPath();g.arc(cx+Math.cos(ca)*skillRadius,cy+Math.sin(ca)*skillRadius*.55,4+skillLv*.5,0,Math.PI*2);g.stroke();}}
    else {g.beginPath();g.arc(cx,cy,skillRadius,phase*.8,phase*.8+Math.PI*1.35);g.stroke();}
    g.restore();
  }
  // Pet effect orbit: companion reads as a system module attached to the jet.
  var pet=petSpriteCache[focusPet&&focusPet.id||save.selectedPet];
  if(pet){var pa=phase*1.15+Math.PI*.35,pr=Math.min(cw,ch)*.28,ppx=cx+Math.cos(pa)*pr,ppy=cy+Math.sin(pa)*pr*.45;g.save();g.globalAlpha=.30;g.strokeStyle=pet.accent;g.lineWidth=1;g.setLineDash([4,6]);g.beginPath();g.ellipse(cx,cy,pr,pr*.45,0,0,Math.PI*2);g.stroke();g.setLineDash([]);g.globalAlpha=.92+.01*petLv;g.drawImage(pet.canvas,ppx-pet.cx*.48,ppy-pet.cy*.48,pet.canvas.width*.48,pet.canvas.height*.48);g.restore();}
...
function renderShop(){
  ...
  var previewRows=dom.shopList.querySelectorAll('.shop-row,.pet-card');
  for(var pr=0;pr<previewRows.length;pr++)previewRows[pr].addEventListener('click',function(ev){
    if(ev.target&&ev.target.closest&&ev.target.closest('button[data-act]'))return;
    var previewBtn=this.querySelector('button[data-act]');
    if(!previewBtn)return;
    shopPreviewFocus={type:previewBtn.getAttribute('data-type'),id:previewBtn.getAttribute('data-id')};
    shopPreviewAnim=[];shopPreviewFiring=0;
    updateShopPreviewInfo();
  });
  if(dom.skillTestBtn){
...
function handleShopAction(){
  ...
  try{
    shopPreviewFocus={type:type,id:id};
    if(act==='buy'){
      ...
      updateShopPreviewInfo();
      if(dom.shopPreview&&dom.shopPreview.parentElement){var bay=dom.shopPreview.parentElement;bay.classList.remove('upgrade-flash');void bay.offsetWidth;bay.classList.add('upgrade-flash');setTimeout(function(){bay.classList.remove('upgrade-flash');},720);}
    }
  }finally{releaseShopLock();}
}
function switchShopTab(tab){
  shopTab=tab;
  var focusMap={gun:save.selectedGun,ship:save.selectedShip,shape:save.selectedShape,skill:save.selectedSkill,pet:save.selectedPet,start:STARTING_UPGRADES[0]&&STARTING_UPGRADES[0].id};
  shopPreviewFocus={type:tab,id:focusMap[tab]||null};
...
function renderAch(){
  ...
  var pages=Math.max(1,Math.ceil(total/ACHIEVEMENTS_PER_PAGE));
  achievementPage=Math.max(0,Math.min(achievementPage,pages-1));
  var from=achievementPage*ACHIEVEMENTS_PER_PAGE;
  var to=Math.min(total,from+ACHIEVEMENTS_PER_PAGE);
  ...
  if(!pager){
    ...
    pager.id='achPager';
    pager.className='ach-pager';
    dom.achList.parentNode.appendChild(pager);
  }
  pager.innerHTML='<button type="button" class="ach-page-btn" data-ach-page="prev" aria-label="Halaman sebelumnya">‹</button><span class="ach-page-label">'+(from+1)+'–'+to+' / '+total+'</span><button type="button" class="ach-page-btn" data-ach-page="next" aria-label="Halaman berikutnya">›</button>';
  pager.querySelector('[data-ach-page="prev"]').disabled=achievementPage<=0;
  pager.querySelector('[data-ach-page="next"]').disabled=achievementPage>=pages-1;
  pager.onclick=function(ev){
    var btn=ev.target.closest('[data-ach-page]');
    if(!btn||btn.disabled)return;
    if(btn.getAttribute('data-ach-page')==='prev')achievementPage--;
    else achievementPage++;
    renderAch();
  };
  var done=achDone();
...
var navDailyBtn=document.getElementById('navDaily');if(navDailyBtn)navDailyBtn.addEventListener('click',function(){initAudio();sfxClick();goScreen('level','levelSelect');});
