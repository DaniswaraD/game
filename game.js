...
function playWipe(direction,onCovered,onDone){
  // Satu wipe aktif pada satu waktu. Panggilan tambahan diabaikan tanpa
  // mengubah state pemanggil; caller bertanggung jawab menentukan state UI.
  ...
  function finish(){
    ...
    wipeEl.style.transform='translate3d('+inX+',0,0)';
    wipeEl.classList.remove('active');
    wipeEl.style.pointerEvents='none';
    wipeLock=false;
    try{if(onDone)onDone();}catch(err){
      console.error('Transition completion error',err);
    }
  }

  try{
    // Block taps on the screen underneath while the wipe is active. The wipe
    // must remain pointer-transparent only after the animation has finished.
    ...
    wipeEl.style.transform='translate3d(0,0,0)';

    setTimeout(function(){
      // Jalankan perubahan layar saat wipe sudah penuh.
      try{if(onCovered)onCovered();}catch(err){
        // Jangan re-throw dari timer: satu error pada layar tujuan tidak boleh
        // mematikan fase OUT atau mengunci input game.
        console.error('Transition covered callback error',err);
      }

      // Jadwalkan OUT setelah callback. Watchdog memastikan lock selalu dilepas
      // walaupun browser sedang mengalami masalah animasi/transition.
      setTimeout(function(){
        try{
          wipeEl.style.transition='transform '+WIPE_MS+'ms cubic-bezier(.65,0,.35,1)';
          wipeEl.style.transform='translate3d('+outX+',0,0)';
        }catch(err){
          console.error('Transition OUT error',err);
          finish();
          return;
        }
        safetyTimer=setTimeout(finish,WIPE_MS+450);
        setTimeout(finish,WIPE_MS);
      },40);
    },WIPE_MS);
  }catch(err){
    console.error('Transition setup error',err);
    finish();
  }
  return true;
}
...
function updateMenuCard(){
  ...
  var last=(typeof save.lastLevelId==='number'&&save.lastLevelId>=0)?LEVELS[save.lastLevelId]:null;
  var canResume=!!(last&&!last.isEndless&&isLevelUnlocked(last.id));
  if(playLabel)playLabel.textContent=canResume?'LANJUTKAN':'MULAI';
  if(playMeta)playMeta.textContent=canResume?('BAB '+last.id+' · '+last.name.toUpperCase()):'PILIH MISI';
  if(playBtn)playBtn.setAttribute('aria-label',canResume?'Lanjutkan misi terakhir':'Mulai bermain');
...
document.getElementById('playBtn').addEventListener('click',function(){
  ...
  initAudio();sfxClick();
  var last=(typeof save.lastLevelId==='number'&&save.lastLevelId>=0)?LEVELS[save.lastLevelId]:null;
  if(last&&!last.isEndless&&isLevelUnlocked(last.id)){startLevel(last,false);return;}
...
window.DS={
  ...
  sfxVictory:sfxVictory,sfxRevive:sfxRevive,sfxSpin:sfxSpin,sfxBuy:sfxBuy,initAudio:initAudio,
