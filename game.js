...
var save={
  ...
  lastLevelId:0,lastPlayedAt:0,dailyStreak:0,dailyLastComplete:''
};
...
var STORY_CHAPTERS=[
  {id:1,title:'SINYA RETAK',desc:'Sinyal misterius membuka jalur perang baru.',levels:[0,1,2],reward:750,xp:180},
  {id:2,title:'SERANGAN BALIK',desc:'Armada musuh mulai mengerahkan senjata berat.',levels:[3,4,5],reward:1800,xp:360},
  {id:3,title:'JANTUNG KEGELAPAN',desc:'Masuk lebih dalam dan hancurkan pusat armada.',levels:[6,7,8],reward:4000,xp:700},
  {id:4,title:'PERANG VORTEX',desc:'Vortex dan Nova menjaga gerbang terakhir.',levels:[9,10,11],reward:8500,xp:1200},
  {id:5,title:'AKHIR SEMESTA',desc:'Gerbang terakhir terbuka. Hadapi rangkaian boss satu per satu.',levels:[12,14,15],reward:20000,xp:2500}
];
var CAMPAIGN_DIALOGUES={
  1:[['COMMAND','Sinyal retak terdeteksi. Armada musuh mulai bergerak.'],['PILOT','Aku akan membuka jalurnya.']],
  2:[['COMMAND','Mereka mengirim unit berat. Jangan beri mereka waktu.'],['PILOT','Kalau begitu kita serang balik.']],
  3:[['SYSTEM','Sumber energi gelap terdeteksi di pusat armada.'],['COMMAND','Hancurkan inti sebelum mereka memperkuat pertahanan.']],
  4:[['PILOT','Vortex sudah di depan mata.'],['COMMAND','Jangan lengah. Penjaga terakhir jauh lebih kuat.']],
  5:[['COMMAND','Gerbang terakhir terbuka. Enam tanda energi terdeteksi.'],['SYSTEM','Boss muncul satu per satu. Boss berikutnya memiliki HP 10x boss sebelumnya.'],['PILOT','Aku akan menghabisinya sampai akhir.']]
};
var campaignDialogQueue=[],campaignDialogIndex=0;
function showCampaignDialog(chapter,after){
  ...
  var lines=CAMPAIGN_DIALOGUES[chapter]||[];if(!box||!lines.length){if(after)after();return;}
  campaignDialogQueue=lines.slice();campaignDialogIndex=0;box.classList.add('on');box._after=after||null;
  box._render=function(){var line=campaignDialogQueue[campaignDialogIndex]||['',''];if(name)name.textContent=line[0];if(text)text.textContent=line[1];if(btn)btn.textContent=campaignDialogIndex===campaignDialogQueue.length-1?'MULAI MISI':'LANJUT';};box._render();
}
...
var LEVEL_STORIES={
  0:[['COMMAND','Sistem navigasi aktif. Pelajari gerakan, tembakan, dan cara bertahan.'],['PILOT','Aku siap.']] ,
  1:[['COMMAND','Sektor pertama aman untuk latihan. Bersihkan jalur dan temukan ritmemu.'],['PILOT','Mulai operasi.']] ,
  2:[['COMMAND','Sinyal asing muncul dari sabuk asteroid. Musuh mulai datang berkelompok.'],['PILOT','Aku akan menembusnya.']] ,
  3:[['COMMAND','Pertahanan sektor kedua runtuh. Mulai sekarang, setiap kesalahan punya harga.'],['PILOT','Kalau begitu jangan sampai salah.']] ,
  4:[['COMMAND','Armada elit memasuki orbit. Mereka tidak akan memberi waktu untuk bernapas.'],['PILOT','Bagus. Aku memang mencari tantangan.']] ,
  5:[['COMMAND','Sinyal darurat berubah menjadi perang terbuka. Sesuatu yang jauh lebih besar sedang bangun.'],['PILOT','Aku terus maju.']] ,
  6:[['COMMAND','Anomali ruang mulai mengganggu sensor. Musuh di depan bukan lagi pasukan biasa.'],['PILOT','Tetap fokus.']] ,
  7:[['COMMAND','DOOM telah membuka gerbang serangan. Tidak ada jalur mundur.'],['PILOT','Berarti jalurnya cuma satu: maju.']] ,
  8:[['COMMAND','Kode 4RROR merusak navigasi. Sistem tidak bisa memprediksi serangan berikutnya.'],['PILOT','Aku akan mengandalkan refleks.']] ,
  9:[['COMMAND','CHAOS menyebar ke seluruh sektor. Struktur arena mulai berubah.'],['PILOT','Aku tidak perlu arena yang tenang.']] ,
  10:[['COMMAND','VORTEX menarik seluruh armada ke pusat badai.'],['PILOT','Kalau pusatnya di sana, aku juga ke sana.']] ,
  11:[['COMMAND','NOVA terdeteksi. Energi musuh melewati batas normal.'],['PILOT','Satu gelombang terakhir sebelum gerbang akhir.']] ,
  12:[['COMMAND','Gerbang Akhir terbuka. Penjaga pertama menunggu. Kalahkan mereka satu per satu.'],['PILOT','Aku akan menutup gerbang ini.']] ,
  14:[['COMMAND','Singularitas runtuh. Ruang di sekitar kapal mulai terlipat.'],['PILOT','Aku sudah terlalu jauh untuk berhenti.']] ,
  15:[['COMMAND','Final Collapse. Semua energi terkonsentrasi pada para penjaga terakhir.'],['PILOT','Selesaikan semuanya.']] 
};
...
function ensureDailyQuest(){
  ...
  if(!q||typeof q!=='object'||q.date!==today){
    ...
    killDirty=true;
  }else if(!q.claimed||typeof q.claimed!=='object')q.claimed={};
  return q;
}
...
function sfxDanger(){beep(180,0.12,'sawtooth',0.045,95);setTimeout(function(){beep(110,0.18,'square',0.035,70);},90);}
...
var LEVELS=[
  ...
  {id:14,name:'SINGULARITAS',chapter:5,unlockCost:0,cardClass:'doom',duration:200,theme:THEME_NOVA,stageLength:28,enemiesPerSecond:10,baseMaxActive:18,maxActiveGrowth:1.2,maxActiveCap:38,hpMult:3.2,dmgMult:6.0,healFreqMult:4.2,throttleRatio:14.0,bossInterval:18,isEndless:false,bossesCanStack:false,spawnRateMult:2.8},
...
function buildBackground(){
  ...
  g.globalAlpha=0.18;
  g.strokeStyle=currentTheme.particles&&currentTheme.particles[0]||'#ffffff';
  g.lineWidth=2;
  if(decoId%5===0){for(var dx=pa.left;dx<pa.right;dx+=120){g.beginPath();g.moveTo(dx,pa.top);g.lineTo(dx+80,pa.bottom);g.stroke();}}
  else if(decoId%5===1){for(var cy=pa.top+70;cy<pa.bottom;cy+=130){g.beginPath();g.arc(W*0.5,cy,Math.min(W,H)*0.22,0,Math.PI*2);g.stroke();}}
  else if(decoId%5===2){for(var dy=pa.top;dy<pa.bottom;dy+=90){g.beginPath();g.moveTo(pa.left,dy);g.lineTo(pa.right,dy+55);g.stroke();}}
  else if(decoId%5===3){for(var cx=pa.left+70;cx<pa.right;cx+=150){g.beginPath();g.arc(cx,H*0.42,34,0,Math.PI*2);g.stroke();g.beginPath();g.arc(cx,H*0.42,52,0,Math.PI*2);g.stroke();}}
  else{g.beginPath();g.moveTo(pa.left,H*0.25);g.quadraticCurveTo(W*.5,H*.05,pa.right,H*.25);g.stroke();g.beginPath();g.moveTo(pa.left,H*.72);g.quadraticCurveTo(W*.5,H*.92,pa.right,H*.72);g.stroke();}
  g.globalAlpha=1;
  g.restore();
...
function buildBossSprite(color,dark,w,flash,style){
  ...
  g.fillStyle=dk;
  if((style||0)%3===1){g.beginPath();g.moveTo(cx,cy-w*.72);g.lineTo(cx+w*.38,cy-w*.25);g.lineTo(cx,cy-w*.45);g.lineTo(cx-w*.38,cy-w*.25);g.closePath();g.fill();}
  else if((style||0)%3===2){for(var di=0;di<6;di++){var da=di*Math.PI/3;g.fillRect(cx+Math.cos(da)*w*.52-3,cy+Math.sin(da)*w*.52-3,6,6);}}
  else {g.fillRect(cx-w/2-8,cy-w*.14,8,w*.28);g.fillRect(cx+w/2,cy-w*.14,8,w*.28);}
...
function spawnBossForFinal(idx){
  ...
  var spawnY=Math.min(edgeTop()+40,maxSpawnY);
  var boss={
...
function playWipe(direction,onCovered,onDone){
  if(wipeLock)return false;
  wipeLock=true;
  var token=++wipeSequence;
  var incoming=(direction==='left')?'-110vw':'110vw';
  var outgoing=(direction==='left')?'110vw':'-110vw';
  var finished=false,coverTimer=null,outTimer=null,safetyTimer=null;
  function clearTimers(){[coverTimer,outTimer,safetyTimer].forEach(function(t){if(t)clearTimeout(t);});coverTimer=outTimer=safetyTimer=null;}
  function finish(){
    if(finished)return;
    ...
    setWipeTransform('translate3d('+incoming+',0,0)');
  ...
  function out(){
    ...
    try{
      ...
      setWipePhase('out');
      setWipeTransition('transform '+WIPE_MS+'ms cubic-bezier(.76,0,.24,1)');
      void wipeEl.offsetWidth;
      requestAnimationFrame(function(){setWipeTransform('translate3d('+outgoing+',0,0)');});
      outTimer=setTimeout(finish,WIPE_MS+90);
    }catch(err){console.error('Transition OUT error',err);finish();}
  }
  try{
    ...
    requestAnimationFrame(function(){
      if(finished||token!==wipeSequence)return;
      setWipeTransition('transform '+WIPE_MS+'ms cubic-bezier(.76,0,.24,1)');
      setWipeTransform('translate3d(0,0,0)');
    });
    coverTimer=setTimeout(function(){
      if(finished||token!==wipeSequence)return;
      wipeEl.classList.remove('phase-in');wipeEl.classList.add('phase-cover');
      setWipePhase('cover');
      ...
      outTimer=setTimeout(out,70);
    },WIPE_MS+30);
    safetyTimer=setTimeout(function(){if(!finished)finish();},WIPE_MS*2+700);
...
function renderStory(){
  ...
  for(var j=0;j<qs.length;j++){var q=qs[j],claimed=!!save.dailyQuest.claimed[q.id],pct=Math.min(100,Math.round(q.progress/q.goal*100));
    qh+='<div class="daily-quest '+(claimed?'done':'')+'"><div class="daily-quest-title">'+q.title+'</div><div class="daily-quest-desc">'+q.desc+'</div><div class="daily-quest-track"><div class="daily-quest-fill" style="width:'+pct+'%"></div></div><div class="daily-quest-meta"><span>'+Math.min(q.progress,q.goal)+' / '+q.goal+'</span><span>+'+q.reward+' P</span><span>+'+q.xp+' XP</span></div>';
...
function renderLevels(){
  ...
  for(var i=0;i<LEVELS.length;i++){
    if(LEVELS[i].isEndless||chapterForLevel(LEVELS[i].id)!==currentChapterTab)continue;
    list.push(LEVELS[i]);
  }
...
function showScreen(name){
  var all=[dom.menuScreen,dom.levelScreen,dom.shopScreen,dom.achScreen,dom.statsScreen,dom.helpScreen,dom.voucherScreen,dom.spinScreen,dom.mpScreen,dom.mpLobbyScreen,dom.challengeScreen,dom.friendsScreen];
  for(var i=0;i<all.length;i++)if(all[i])all[i].classList.remove('on');
  refreshAll();
  if(name==='menu'){dom.menuScreen.classList.add('on');try{if(window.MP_refreshGlobalChampion)window.MP_refreshGlobalChampion();}catch(e){}}
...
window.addEventListener('beforeunload',function(){persist();v111RefreshCommand();});
...
function v111SafeLevel(id){id=Number(id);if(!isFinite(id))return null;for(var i=0;i<LEVELS.length;i++)if(Number(LEVELS[i].id)===id)return LEVELS[i];return null;}
function v111RefreshCommand(){var meta=document.getElementById('v111ResumeMeta'),st=document.getElementById('v111SaveState');var lvl=v111SafeLevel(save.lastLevelId);if(meta)meta.textContent=lvl?('BAB '+chapterForLevel(lvl.id)+' • '+(lvl.name||('LEVEL '+lvl.id))):'Belum ada misi terakhir';if(st)st.textContent=storageAvailable?'LOCAL SAVE • SIAP':'LOCAL SAVE • TERBATAS';}
function v111Resume(){var lvl=v111SafeLevel(save.lastLevelId);if(!lvl){showToast('Belum ada misi terakhir.','info');return false;}initAudio();sfxClick();return startLevel(lvl,false);}
function v111Daily(){initAudio();sfxClick();goScreen('level','levelSelect');setTimeout(function(){var c=document.getElementById('dailyOpsCard');if(c)c.scrollIntoView({behavior:'smooth',block:'center'});},620);}
function v111Sync(){initAudio();var b=document.getElementById('v111SyncBtn'),st=document.getElementById('v111SaveState');if(b)b.disabled=true;if(st)st.textContent='CLOUD SYNC • MENYIAPKAN';try{persist();if(window.MP_forceCloudSync)window.MP_forceCloudSync();else pushProfileToFirebase();}catch(e){}setTimeout(function(){if(b)b.disabled=false;if(st)st.textContent='LOCAL SAVE • SIAP';},1000);}
function v111BindCommand(){var r=document.getElementById('v111ResumeBtn'),d=document.getElementById('v111DailyBtn'),b=document.getElementById('v111SyncBtn');if(r)r.addEventListener('click',v111Resume);if(d)d.addEventListener('click',v111Daily);if(b)b.addEventListener('click',v111Sync);v111RefreshCommand();}
...
v111RefreshCommand:v111RefreshCommand,
  ...
  renderShop:renderShop,renderLevels:renderLevels,edgeLeft:edgeLeft,edgeRight:edgeRight,edgeTop:edgeTop,
