...
function validateSave(){
  ...
  if(typeof save.mpTeamKills!=='number'||!isFinite(save.mpTeamKills))save.mpTeamKills=0;
}
...
var WIPE_MS=180,wipeLock=false;
...
function renderStory(){
  if(!dom.storyList||!dom.dailyQuestList)return;
  var html='';
  for(var i=0;i<STORY_CHAPTERS.length;i++){var ch=STORY_CHAPTERS[i],un=storyChapterUnlocked(i),done=storyChapterDone(ch);
    html+='<div class="story-card '+(!un?'locked ':'')+(done?'done':'')+'">';
    html+='<div class="story-title">BAB '+ch.id+' — '+ch.title+'</div><div class="story-desc">'+ch.desc+'</div>';
    html+='<div class="story-meta"><span class="story-pill">'+ch.levels.length+' Misi</span><span class="story-pill">+'+ch.reward.toLocaleString('id-ID')+' poin</span><span class="story-pill">+'+ch.xp+' XP</span></div>';
    if(done&&!save.storyClaimed[ch.id])html+='<button class="story-action" data-claim-story="'+i+'">AMBIL REWARD BAB</button>';
    else if(done)html+='<div class="story-pill" style="margin-top:10px;display:inline-block">✓ REWARD DIAMBIL</div>';
    else if(!un)html+='<div class="story-pill" style="margin-top:10px;display:inline-block">🔒 Selesaikan bab sebelumnya</div>';
    else html+='<button class="story-action" data-start-story="'+ch.levels[0]+'">MULAI BAB</button>';
    html+='</div>';
  }
  dom.storyList.innerHTML=html;
  ...
  for(var j=0;j<qs.length;j++){var q=qs[j],claimed=!!save.dailyQuest.claimed[q.id],pct=Math.min(100,Math.round(q.progress/q.goal*100));
    qh+='<div class="story-card '+(claimed?'done':'')+'"><div class="story-title">'+q.title+'</div><div class="story-desc">'+q.desc+'</div><div style="height:10px;background:#eee;border:2px solid #242438;border-radius:8px;overflow:hidden"><div style="height:100%;width:'+pct+'%;background:linear-gradient(90deg,#3ddc97,#9b6bff)"></div></div><div class="story-meta" style="margin-top:8px"><span class="story-pill">'+Math.min(q.progress,q.goal)+' / '+q.goal+'</span><span class="story-pill">+'+q.reward+' poin</span><span class="story-pill">+'+q.xp+' XP</span></div>';
    if(claimed)qh+='<div class="story-pill" style="margin-top:10px;display:inline-block">✓ SELESAI</div>';
    else qh+='<button class="story-action" data-claim-daily="'+q.id+'" '+(q.progress<q.goal?'disabled':'')+'>AMBIL REWARD</button>';
    qh+='</div>';
  }
  dom.dailyQuestList.innerHTML=qh;
  var sb=dom.storyList.querySelectorAll('[data-start-story]');for(var a=0;a<sb.length;a++)sb[a].addEventListener('click',function(){initAudio();sfxClick();var L=LEVELS[Number(this.getAttribute('data-start-story'))];if(L)startLevel(L,false);});
  var sc=dom.storyList.querySelectorAll('[data-claim-story]');for(var b=0;b<sc.length;b++)sc[b].addEventListener('click',function(){initAudio();if(claimStoryReward(Number(this.getAttribute('data-claim-story')))){sfxBuy();showToast('Reward bab diterima!','success');renderStory();refreshAll();}});
  var dq=dom.dailyQuestList.querySelectorAll('[data-claim-daily]');for(var c=0;c<dq.length;c++)dq[c].addEventListener('click',function(){initAudio();if(claimDailyQuest(this.getAttribute('data-claim-daily'))){sfxBuy();showToast('Misi harian selesai!','success');renderStory();refreshAll();}});
}
