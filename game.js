...
function updateShopPreviewInfo(){
  ...
  else {item=STARTING_UPGRADES[0];stats='SYSTEM BONUS ACTIVE';}
...
function renderShopPreview(dt){
  ...
  var focusSkill=(focusType==='skill'&&focusId)?findSkill(focusId):findSkill(save.selectedSkill);
  var focusPet=(focusType==='pet'&&focusId)?findPet(focusId):findPet(save.selectedPet);
  ...
  var fxPower=1+Math.max(shipLv,gunLv,skillLv,petLv)*0.16;
  ...
  if(glow){g.save();g.globalAlpha=.30+.04*Math.max(shipLv,gunLv);g.drawImage(glow.normal.canvas,cx-glow.R*.8,cy-glow.R*.8,glow.R*1.6,glow.R*1.6);g.restore();}
