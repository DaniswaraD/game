...
function buildPetSprite(pet){
  ...
  g.fillStyle=accent;
  if(pet.id==='scout'||pet.id==='pulse'||pet.id==='apex'){
...
function initPixiFxLayer(){
  if(PERF_MODE||pixiFxReady||pixiFxLoading||!window.DS_LIBS||!window.DS_LIBS.loadPixi)return;
  pixiFxLoading=true;
...
function updatePixiFx(){
  if(!pixiFxReady||!pixiFxPool.length)return;
  var used=0;
...
function drawParticles(){
  for(var i=0;i<particles.length;i++){
    ...
    if(p.text!==null&&p.text!==undefined){
      ...
      if(pixiFxReady&&pixiHex(p.color))continue;
...
function showScreen(name){
  ...
  refreshAll();
  if(name==='menu'){setMusicMode('menu');dom.menuScreen.classList.add('on');try{if(window.MP_refreshGlobalChampion)window.MP_refreshGlobalChampion();}catch(e){}}
