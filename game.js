...
var STORY_CHAPTERS=[
  ...
  {id:5,title:'AKHIR SEMESTA',desc:'Pertarungan terakhir melawan penguasa akhir.',levels:[12],reward:20000,xp:2500}
];
...
function ensureDailyQuest(){
  var today=new Date().toISOString().slice(0,10),q=save.dailyQuest;
...
function killBoss(boss){
  ...
  checkAchievements();
  persist();
  if(dom.storyScreen&&dom.storyScreen.classList.contains('on'))renderStory();
}
...
var dom={
  ...
  storyScreen:document.getElementById('storyScreen'),storyList:document.getElementById('storyList'),dailyQuestList:document.getElementById('dailyQuestList'),
...
function showScreen(name){
  var all=[dom.menuScreen,dom.levelScreen,dom.shopScreen,dom.achScreen,dom.statsScreen,dom.helpScreen,dom.voucherScreen,dom.spinScreen,dom.mpScreen,dom.mpLobbyScreen,dom.challengeScreen,dom.leaderScreen,dom.friendsScreen,dom.storyScreen];
  ...
  else if(name==='story'){if(dom.storyScreen)dom.storyScreen.classList.add('on');renderStory();}
}
...
if(navStoryBtn)navStoryBtn.addEventListener('click',function(){initAudio();sfxClick();goScreen('story','storyMenu');});
var storyBackBtn=document.getElementById('storyBack');
if(storyBackBtn)storyBackBtn.addEventListener('click',function(){initAudio();sfxClick();goScreen('menu','menu');});
...
dom.resetAll.addEventListener('click',function(){
  ...
  showConfirm('Hapus semua progres? Semua poin, skill, kapal, senjata, upgrade, pencapaian, dan tingkat terbuka akan hilang.',function(){
    ...
    save.trophies=0;save.mpGifts=0;save.ownedSkills=[];save.selectedSkill=null;save.storyClaimed={};save.dailyQuest={date:new Date().toISOString().slice(0,10),killsStart:0,bossStart:0,winsStart:0,claimed:{}};
