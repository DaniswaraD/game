...
function accountLogin(){
  if(!firebaseReady||!firebase.auth){accountStatus('Firebase Authentication belum siap.',true);return;}
  var username=normalizeUsername(document.getElementById('accountUsername').value);
  ...
  accountStatus('Memeriksa akun...');
  firebase.auth().signInWithEmailAndPassword(usernameEmail(username),password).then(function(result){
    DS().save.globalId=result.user.uid;
    DS().save.playerName=username.toUpperCase();
    DS().persist();
    accountStatus('Berhasil masuk.');
    refreshAccountUI();
    var accountBox=document.getElementById('accountBox');if(accountBox)accountBox.classList.add('hidden');
    syncUserProfile();
  }).catch(function(err){
    accountStatus(authErrorMessage(err,'Login gagal.'),true);
  });
}
function accountRegister(){
  ...
  accountStatus('Membuat akun...');
  return firebase.auth().createUserWithEmailAndPassword(usernameEmail(username),password).then(function(result){
    return db.ref('usernames/'+username).set(result.user.uid).then(function(){return result;}).catch(function(err){
      return result.user.delete().catch(function(){}).then(function(){
        if(err&&err.code==='PERMISSION_DENIED')throw {code:'auth/email-already-in-use',message:'Username sudah terdaftar atau tidak dapat digunakan.'};
        throw err;
      });
    });
    ...
    DS().save.playerName=username.toUpperCase();
    DS().persist();
    return db.ref('users/'+result.user.uid).set({id:result.user.uid,username:username,name:username.toUpperCase(),kills:DS().save.kills||0,totalKills:DS().save.totalKills||0,level:DS().save.level||1,xp:DS().save.xp||0,trophies:DS().save.trophies||0,lastSeen:nowTs()});
  }).then(function(){
    accountStatus('Akun berhasil dibuat. Login tersimpan di perangkat ini.');
    refreshAccountUI();
    var accountBox=document.getElementById('accountBox');if(accountBox)accountBox.classList.add('hidden');
    syncUserProfile();
  }).catch(function(err){
...
function accountLogout(){
  ...
  firebase.auth().signOut().then(function(){
    ...
    DS().showToast('Akun keluar','info',2200);
  });
}
...
function syncUserProfile(){
  ...
  if(!au)return Promise.resolve(false);
  s.globalId=au.uid;
  if(!mp.userRef||mp.userRef.key!==s.globalId)mp.userRef=db.ref('users/'+s.globalId);
  var profile=getUserProfile();
  return mp.userRef.update(profile).then(function(){
    mp.firebaseConnected=true;
    if(!mp.userSyncTimer){
      mp.userSyncTimer=setInterval(function(){
        var u=currentAuthUser();
        var id=DS().save.globalId;
        if(db&&u&&id&&u.uid===id){
          db.ref('users/'+id).update(getUserProfile()).catch(function(err){
            mp.firebaseConnected=false;
          });
        }
      },USER_SYNC_INTERVAL);
    ...
    DS().showToast('Sinkronisasi profil gagal: '+code,'info',3200);
    return false;
  });
}
