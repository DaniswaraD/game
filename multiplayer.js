...
function acceptFriend(uid,uname){
  ...
  var myName=(DS().save.playerName||'PLAYER').toUpperCase();
  var updates={};
  // Atomic two-way friendship. Rules v5 allow the receiver to write the
  // reciprocal side while the original request still exists.
  updates['friends/'+myId+'/'+uid]={id:uid,name:uname||'-',ts:nowTs()};
  updates['friends/'+uid+'/'+myId]={id:myId,name:myName,ts:nowTs()};
  ...
  db.ref().update(updates).then(function(){
    ...
    DS().showToast('Gagal terima teman. Coba lagi.','error');
  });
}
