...
var mp={
  ...
  firebaseConnected:false,sharedCombat:false,globalKillsTotal:0
};
...
function finishStartupLoading(){
  ...
  setTimeout(function(){ov.classList.add('hide');},420);
}
var startupFinished=false;
function enterGameAfterAuth(){
  if(startupFinished)return;
  startupFinished=true;
  startupLoading('MEMUAT DATA PEMAIN');
  var wait=Promise.resolve();
  try{
    if(typeof syncUserProfile==='function')wait=Promise.race([Promise.resolve(syncUserProfile()),new Promise(function(resolve){setTimeout(resolve,3500);})]);
  }catch(e){}
  wait.then(function(){
    refreshAccountUI();
    updateAccountGate();
...
function forceReloginIfNeeded(user){
  var key='danisShooter_session_epoch';
  var local=localStorage.getItem(key)||'';
  if(local!==FORCE_RELOGIN_EPOCH){
    localStorage.setItem(key,FORCE_RELOGIN_EPOCH);
    if(user){try{firebase.auth().signOut();}catch(e){}return true;}
  }
  return false;
}
...
function adminHash(v){var h=2166136261;v=String(v||'');for(var i=0;i<v.length;i++){h^=v.charCodeAt(i);h=Math.imul(h,16777619);}return (h>>>0).toString(16);}
var ADMIN_CODE_HASH='';
...
function deleteOwnAccount(){
  ...
  u.reauthenticateWithCredential(cred).then(function(){
    ...
    DS().save.globalId='';DS().save.playerName='';DS().persist();DS().showToast('Akun dihapus.','success',3000);
  ...
  initFirebase().then(function(ok){
    if(ok&&firebase.auth){
      firebase.auth().onAuthStateChanged(function(user){
        if(user){
          if(forceReloginIfNeeded(user))return;
        ...
        if(user){
          ...
          var ov=document.getElementById('loadingOverlay');if(ov)ov.classList.add('hide');
          updateAccountGate();
