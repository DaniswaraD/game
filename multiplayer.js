...
function renderLobby(){
  ...
  renderModeGrid();renderLobbyInviteFriends();renderChat();updateLobbyConnectionUI();
}
...
function renderModeGrid(){
  ...
  for(var i=0;i<MP_MODES.length;i++){
    ...
    html+='<button title="'+esc(m.label)+' • Level '+m.level+'" class="mp-mode-btn'+on+lk+'" data-mode="'+m.id+'"'+(mp.isHost?'':' disabled')+'><b>'+m.label+'</b><small>LV '+m.level+'</small></button>';
  }
  grid.innerHTML=html;
}
