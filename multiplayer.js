...
function joinGroupByCode(code,nameInput){
  ...
  if(!db){DS().showToast('Database belum siap. Coba lagi.','error',4200);return;}
  var user=currentAuthUser();
  if(!user){DS().showToast('Sesi akun belum siap. Masuk ulang lalu coba lagi.','error',4200);return;}
  var name=currentUsername();
  code=String(code||'').trim().toUpperCase().replace(/[^A-Z0-9]/g,'');
  if(code.length!==6){DS().showToast('Kode grup harus 6 karakter','error');return;}
  if(!name){DS().showToast('Nama akun belum siap','error');return;}
  DS().save.playerName=name;
  DS().save.globalId=user.uid;
  DS().persist();
  syncUserProfile();
  var ref=db.ref('rooms/'+code);
  var playerRef=ref.child('players').child(user.uid);
  return ref.once('value').then(function(snap){
    if(!snap.exists())throw {code:'room/not-found',message:'Grup tidak ditemukan'};
    var data=snap.val()||{};
    var state=String(data.state||'lobby');
    if(state==='playing')throw {code:'room/in-progress',message:'Grup sedang bermain'};
    if(state!=='lobby'&&state!=='ended')throw {code:'room/unavailable',message:'Grup belum dapat dimasuki'};
    var players=data.players||{};
    var hostId=data.hostId||null;
    if(!hostId||!players[hostId])throw {code:'room/no-host',message:'Host grup sudah keluar'};
    if(players[user.uid])throw {code:'room/already-member',message:'Kamu sudah berada di grup ini'};
    var nameKey=name.toUpperCase(),count=0;
    for(var k in players){
      if(!players.hasOwnProperty(k))continue;
      count++;
      if(String(players[k]&&players[k].name||'').toUpperCase()===nameKey)throw {code:'room/name-used',message:'Nama sudah dipakai di grup'};
    }
    if(count>=MAX_PLAYERS)throw {code:'room/full',message:'Grup sudah penuh'};
    var me={
      ...
      skill:DS().save.selectedSkill||'',
      pet:DS().save.selectedPet||'',
      alive:true,kills:0,kp:DS().save.kills,
      level:DS().save.level,xp:DS().save.xp,
      joinedAt:nowTs(),lastSeen:nowTs(),
      lobbyX:0.5,posX:0.5,posHp:1,ready:false,online:true
    };
    /* Important: a joining player may write only their own players/<uid> node under the rules. */
    return playerRef.set(me).then(function(){return ref.once('value');});
  }).then(function(finalSnap){
    var data=finalSnap.val()||{},players=data.players||{};
    if(!players[user.uid])throw {code:'room/join-not-confirmed',message:'Server belum mengonfirmasi keanggotaan'};
    mp.myId=user.uid;mp.isHost=(data.hostId===user.uid);mp.hostId=data.hostId||null;
    mp.roomCode=code;mp.selectedMode=data.mode||1;mp.lastWrittenKp=null;mp.lobbyX=0.5;
    mp.gameEnded=false;mp.globalMode=false;mp.prevRoomState=data.state||'lobby';mp.startRequested=false;mp.finalSent=false;mp.ready=false;
    attachListeners(code,user.uid,false);
    DS().showScreen('mpLobby');
    var codeEl=document.getElementById('mpRoomCode');if(codeEl)codeEl.textContent=code;
    renderLobby();DS().sfxMP();
    return true;
  }).catch(function(err){
    var codeErr=String(err&&err.code||'').toLowerCase();
    var msg=err&&err.message?String(err.message):'';
    if(codeErr.indexOf('permission')>=0||codeErr.indexOf('permission-denied')>=0){
      DS().showToast('Firebase menolak masuk grup. Pastikan Firebase Rules yang dipakai adalah rules versi game terbaru.','error',6500);
    }else if(codeErr==='room/not-found')DS().showToast('Grup tidak ditemukan','error');
    else if(codeErr==='room/full')DS().showToast('Grup sudah penuh','error');
    else if(codeErr==='room/name-used')DS().showToast('Nama sudah dipakai di grup','error');
    else if(codeErr==='room/already-member')DS().showToast('Kamu sudah ada di grup ini','info');
    else if(codeErr==='room/in-progress')DS().showToast('Grup sedang bermain','error');
    else if(codeErr==='room/no-host')DS().showToast('Host grup sudah keluar','error');
    else DS().showToast('Gagal masuk grup'+(msg?': '+msg:''),'error',5200);
    return false;
  });
}
