...
var mp={
  ...
  firebaseConnected:false,sharedCombat:false
};
...
function enterGameAfterAuth(){
  ...
  // Auth check is the only startup gate. Do not block the game on a profile write.
  refreshAccountUI();
  updateAccountGate();
  var box=document.getElementById('accountBox');if(box)box.classList.add('hidden');
  if(DS().save.playerName)DS().refreshAll();
  finishStartupLoading();
  // Sync online profile in the background so Firebase latency never delays entering the game.
...
function renderLobby(){
  ...
  var html='<div class="global-stat-board"><div class="global-stat-head"><b>'+players.length+'/'+MAX_PLAYERS+'</b> PEMAIN <span>'+ (mp.isHost?'HOST':'ROOM') +'</span></div>';
  for(var i=0;i<players.length;i++){
    var pl=players[i];
    html+='<div class="global-stat-row"><span class="gs-rank">'+(i+1)+'</span><span class="gs-name">'+esc((pl.name||'-').toUpperCase())+(pl.id===hostId?' ★':'')+'</span><span class="gs-kill">K '+fmtK(pl.kills||0)+'</span><span class="gs-kp">'+fmtK(pl.kp||0)+' KP</span>';
  ...
  if(wait){var total=players.length;wait.textContent=total<2?'Menunggu pemain... ('+total+'/'+MAX_PLAYERS+') - Butuh minimal 2':total+' / '+MAX_PLAYERS+' pemain di grup';}
  var startBtn=document.getElementById('mpStartBtn');
  if(startBtn){var canStart=mp.isHost&&players.length>=2&&players.length<=MAX_PLAYERS;startBtn.disabled=!canStart;startBtn.textContent=mp.isHost?'START':'MENUNGGU HOST';}
  var mt=document.getElementById('mpModeTitle');if(mt)mt.textContent=mp.isHost?'PILIH MODE (HOST)':'MODE DIPILIH HOST';
  renderModeGrid();renderLobbyInviteFriends();renderChat();
}
...
function renderGlobalLobby(){
  ...
  if(!lobby)return;
  var html='<div class="global-stat-board"><div class="global-stat-head"><b>'+players.length+'</b> ONLINE <span>TEAM KILL <b>'+fmtK(mp.globalKillsTotal||0)+'</b></span></div>';
  for(var i=0;i<Math.min(players.length,12);i++){var p=players[i];html+='<div class="global-stat-row"><span class="gs-rank">'+(i+1)+'</span><span class="gs-name">'+esc((p.name||'-').toUpperCase())+'</span><span class="gs-kill">'+fmtK(p.kills||0)+'</span><span class="gs-kp">'+fmtK(p.kp||0)+' KP</span></div>';}
...
function updateTopStats(){
  var el=document.getElementById('mpStatsBody');if(!el)return;
  var players=sortPlayersByKills();if(players.length===0){el.innerHTML='<div class="mp-stat-empty">Menunggu pemain...</div>';return;}
  var myId=mp.globalMode?DS().save.globalId:mp.myId,teamKills=mp.globalKillsTotal||0;
  var maxKills=1;for(var m=0;m<players.length;m++)maxKills=Math.max(maxKills,players[m].kills||0);
  var html='<div class="mp-stat-summary"><span>TEAM KILL <b>'+teamKills+'</b></span><span>ONLINE <b>'+players.length+'</b></span></div>';
  for(var j=0;j<players.length&&j<8;j++){
    var pl=players[j],kp=pl.kills||0,mine=pl.id===myId?' me':'',pct=Math.max(4,Math.round(kp/maxKills*100));
    html+='<div class="mts-row'+mine+'"><div class="mts-dot"></div><div class="mts-name">'+esc((pl.name||'-').toUpperCase())+'</div><div class="mts-bar"><div class="mts-fill" style="width:'+pct+'%"></div></div><div class="mts-kill">'+kp+'</div></div>';
  }
  el.innerHTML=html;
}

function drawOtherPlayersInGame(){
  if(!MPApi().active)return;var ctx=getGameCtx();if(!ctx)return;
  var W=DS().getW(),H=DS().getH(),BS=DS().BASE_SIZE,border=DS().BORDER,cache=mp.globalMode?mp.globalPlayers:PC(),myId=mp.globalMode?DS().save.globalId:mp.myId;
  var ids=[];for(var k in cache)if(k!==myId&&cache[k]&&cache[k].alive!==false)ids.push(k);
  for(var ii=0;ii<ids.length;ii++){var pl=cache[ids[ii]];if(pl.posX===undefined)continue;var ox=pl.posX*W-BS/2,elL=DS().edgeLeft(),elR=DS().edgeRight();if(ox<elL)ox=elL;if(ox+BS>elR)ox=elR-BS;var lane=(ii-(ids.length-1)/2)*42,oy=H-border-155-Math.min(70,Math.abs(lane));var ship=DS().findShip(pl.ship||'default'),shape=DS().findShape(pl.shape||'square'),key=ship.id+'_'+shape.id,cacheSpr=DS().playerSpriteCache(),spr=cacheSpr[key]||cacheSpr[ship.id+'_square']||cacheSpr['default_square'],glow=DS().playerGlowCache()[ship.id],cx=ox+BS/2,cy=oy+BS/2,hpRatio=clamp(pl.posHp===undefined?1:pl.posHp,0,1);
    ctx.save();ctx.globalAlpha=.95;ctx.fillStyle='rgba(255,255,255,.92)';ctx.fillRect(ox-3,oy-25,BS+6,6);ctx.fillStyle='rgba(36,36,56,.35)';ctx.fillRect(ox-2,oy-24,BS+4,4);ctx.fillStyle=hpRatio<.35?'#ff6b4a':'#3ddc97';ctx.fillRect(ox,oy-23,BS*hpRatio,3);ctx.restore();
    if(glow&&glow.normal&&glow.normal.canvas){ctx.save();ctx.globalAlpha=.22;ctx.drawImage(glow.normal.canvas,cx-glow.R,cy-glow.R);ctx.restore();}
    ctx.save();ctx.globalAlpha=.98;if(spr&&spr.normal&&spr.normal.canvas)ctx.drawImage(spr.normal.canvas,ox-spr.pad,oy-spr.pad);else{ctx.fillStyle=ship.body||'#67c7f0';ctx.fillRect(ox,oy,BS,BS);}ctx.restore();
    ctx.save();ctx.globalAlpha=.92;ctx.font='bold 10px Fredoka,sans-serif';ctx.textAlign='center';ctx.lineWidth=3;ctx.strokeStyle='rgba(36,36,56,.9)';ctx.strokeText((pl.name||'-').toUpperCase(),cx,oy-31);ctx.fillStyle='#fff';ctx.fillText((pl.name||'-').toUpperCase(),cx,oy-31);ctx.restore();
  }
}
...
function gameSyncTick(dt){
  ...
  ref.update({
    ...
    kills:MPApi().myKillCount||0,
    teamKills:mp.globalKillsTotal||0,
    kp:DS().save.kills,
    level:DS().save.level,
    xp:DS().save.xp,
    ship:DS().save.selectedShip,
...
function tryAdminCode(code){
  if(String(code||'').toUpperCase()!=='IMANADMINBRO404')return false;
  openAdminPanel();
  return true;
}
function openAdminPanel(){
  ...
  db.ref('admins/'+uid).once('value').then(function(s){if(s.val()!==true){DS().showToast('Akun ini belum ditetapkan sebagai admin di Firebase.','error',4500);return;} openAdminPanelUI();});
}
function openAdminPanelUI(){
  var old=document.getElementById('adminPanelScreen');
  if(old)old.remove();
  var el=document.createElement('div');el.id='adminPanelScreen';el.className='screen on admin-panel-screen';
  el.innerHTML='<div class="topbar"><button class="icon-btn" id="adminBack">Kembali</button><div class="kill-chip">ADMIN PANEL</div></div><div class="scroll-area"><div class="inner"><div class="admin-warning">Panel client hanya untuk kontrol game. Password pengguna tidak dapat dan tidak boleh ditampilkan.</div><div class="admin-actions"><button id="adminResetSessions">PAKSA SEMUA LOGIN ULANG</button><button id="adminResetLeaderboard">RESET NAMA LEADERBOARD</button></div><div id="adminUserList" class="admin-user-list"><div class="mp-room-empty">Memuat pemain...</div></div></div></div>';
  document.body.appendChild(el);
  document.getElementById('adminBack').addEventListener('click',function(){el.remove();showAdminGate();});
  document.getElementById('adminResetSessions').addEventListener('click',function(){resetAllSessions();});
  document.getElementById('adminResetLeaderboard').addEventListener('click',function(){resetLeaderboardNames();});
  renderAdminUsers();
}
function showAdminGate(){
  var box=document.createElement('div');box.id='adminGate';box.className='admin-gate';
  box.innerHTML='<div class="admin-gate-box"><div class="admin-gate-title">ADMIN PANEL</div><input id="adminGateCode" type="password" maxlength="32" placeholder="KODE ADMIN" autocomplete="off"><div class="admin-gate-actions"><button id="adminGateEnter">MASUK</button><button id="adminGateCancel">BATAL</button></div></div>';
  document.body.appendChild(box);
  document.getElementById('adminGateEnter').addEventListener('click',function(){var c=document.getElementById('adminGateCode').value;if(c==='IMANADMINBRO404'){box.remove();openAdminPanel();}else{DS().showToast('Kode admin salah','error');}});
  document.getElementById('adminGateCancel').addEventListener('click',function(){box.remove();});
}
function resetAllSessions(){
  if(!db)return;
  db.ref('system/sessionEpoch').set(FORCE_RELOGIN_EPOCH+'-'+Date.now()).then(function(){return db.ref('global_arena/players').remove();}).then(function(){DS().showToast('Sesi aktif ditandai untuk login ulang.','success',3000);}).catch(function(){DS().showToast('Gagal mengubah sesi.','error');});
}
function resetLeaderboardNames(){
  if(!db)return;
  if(!confirm('Hapus semua nama dari leaderboard? Data akun, KP, dan progres TIDAK dihapus. Pemain yang sedang login akan muncul lagi setelah sinkronisasi.'))return;
  db.ref('leaderboard').once('value').then(function(snap){
    var updates={};
    snap.forEach(function(child){updates['leaderboard/'+child.key]=null;});
    if(!Object.keys(updates).length){DS().showToast('Leaderboard sudah kosong.','info',2500);return null;}
    return db.ref().update(updates);
  }).then(function(){
    DS().showToast('Nama leaderboard berhasil direset. Data pemain tetap aman.','success',3500);
    var list=document.getElementById('leaderList');if(list)list.innerHTML='<div style=\"text-align:center;padding:40px;color:#8a8aa3;font-weight:600;font-size:12px;\">Belum ada pemain terdaftar</div>';
    renderAdminUsers();
  }).catch(function(err){
    console.error('resetLeaderboardNames',err);
    DS().showToast('Gagal mereset leaderboard.','error',3500);
  });
}
function renderAdminUsers(){
  var list=document.getElementById('adminUserList');if(!list||!db)return;
  db.ref('global_arena/players').once('value').then(function(snap){
    var d=snap.val()||{},arr=[];
    for(var id in d){var p=d[id];if(nowTs()-(p.lastSeen||0)<PLAYER_STALE_MS)arr.push({id:id,p:p});}
    arr.sort(function(a,b){return (b.p.lastSeen||0)-(a.p.lastSeen||0);});
    if(!arr.length){list.innerHTML='<div class="mp-room-empty">Tidak ada pemain aktif.</div>';return;}
    var html='';
    for(var i=0;i<arr.length;i++){var p=arr[i].p,id=arr[i].id;html+='<div class="admin-user"><div><b>'+esc((p.name||'-').toUpperCase())+'</b><small>KP '+fmtK(p.kp||0)+'</small></div><div class="admin-user-actions"><button data-kp="'+id+'" data-v="1000">+1K</button><button data-kp="'+id+'" data-v="-1000">-1K</button><button data-kick="'+id+'">KICK</button><button data-remove="'+id+'">HAPUS DATA</button></div></div>';}
    list.innerHTML=html;
    list.querySelectorAll('[data-kp]').forEach(function(b){b.addEventListener('click',function(){adminAdjustKP(this.getAttribute('data-kp'),Number(this.getAttribute('data-v')));});});
    list.querySelectorAll('[data-kick]').forEach(function(b){b.addEventListener('click',function(){adminKick(this.getAttribute('data-kick'));});});
...
function adminAdjustKP(uid,delta){
  if(!db)return;
  db.ref('users/'+uid).once('value').then(function(snap){var cur=snap.val()||{},next=Math.max(0,Number(cur.kills||0)+delta);var updates={};updates['users/'+uid+'/kills']=next;updates['leaderboard/'+uid+'/kills']=next;return db.ref().update(updates);}).then(function(){renderAdminUsers();}).catch(function(){DS().showToast('KP tidak dapat diubah. Pastikan UID admin ada di /admins.','error');});
}
function adminKick(uid){
  if(!db)return;
  db.ref('global_arena/players/'+uid).remove().then(function(){DS().showToast('Pemain dikeluarkan dari arena.','success');renderAdminUsers();});
}
function adminRemoveData(uid){
  if(!db||uid===DS().save.globalId)return;
  if(!confirm('Hapus data online akun ini? Ini tidak dapat menghapus password/Auth akun dari browser.'))return;
  var updates={};
  updates['users/'+uid]=null;
  updates['leaderboard/'+uid]=null;
  updates['friends/'+uid]=null;
  updates['friend_requests/'+uid]=null;
  updates['friend_sent/'+uid]=null;
  updates['global_arena/players/'+uid]=null;
  updates['account_flags/'+uid+'/removed']=true;
  db.ref().update(updates).then(function(){DS().showToast('Data akun dihapus dari database game.','success');renderAdminUsers();}).catch(function(){DS().showToast('Gagal menghapus data akun.','error');});
}
