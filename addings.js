...
function doSpin(tierId){
  ...
  try{spinSnapshot=JSON.parse(JSON.stringify(s));}catch(err){spinSnapshot=null;}

  s.kills-=tier.cost;
  if(!s.spins)s.spins=0;
  s.spins++;
  // UI/storage refresh must never abort the spin after the cost has been charged.
  // The actual reward transaction below remains protected by the interval finally.
  try{DS().persist();}catch(err){console.error('Spin initial persist error',err);}
  try{DS().refreshHeaderKills();}catch(err){console.error('Spin initial header refresh error',err);}
  try{DS().refreshProfile();}catch(err){console.error('Spin initial profile refresh error',err);}
  try{DS().updateMenuCard();}catch(err){console.error('Spin initial menu refresh error',err);}
  ...
  var tid=setInterval(function(){
    try{
      ...
      var res;
      try{
        res=applyReward(reward);
      }catch(rewardErr){
        // The cost was charged before the animation. If reward application itself
        // fails, restore the exact pre-spin save snapshot so no KP/progress is lost.
        if(spinSnapshot){
          var live=DS().save;
          for(var sk in live)if(Object.prototype.hasOwnProperty.call(live,sk))delete live[sk];
          for(var rk in spinSnapshot)live[rk]=spinSnapshot[rk];
        }else{
          DS().save.kills+=tier.cost;
          DS().save.spins=Math.max(0,(DS().save.spins||1)-1);
        }
        try{DS().persist();}catch(e){}
        try{DS().refreshHeaderKills();}catch(e){}
        try{DS().refreshProfile();}catch(e){}
        try{DS().updateMenuCard();}catch(e){}
        DS().showToast('Spin gagal diproses. KP dikembalikan.','error',3200);
        console.error('Spin reward error',rewardErr);
        return;
      }
      // Reward application succeeded. UI errors after this point must never
      // roll back the already-earned reward.
      try{DS().checkAchievements();}catch(err){console.error('Spin achievement refresh error',err);}
      try{DS().persist();}catch(err){console.error('Spin persist error',err);}
      try{DS().refreshHeaderKills();}catch(err){console.error('Spin header refresh error',err);}
      try{DS().refreshProfile();}catch(err){console.error('Spin profile refresh error',err);}
      try{DS().updateMenuCard();}catch(err){console.error('Spin menu refresh error',err);}
      try{if(window.AD_spinRender)window.AD_spinRender();}catch(err){console.error('Spin button render error',err);}
      try{showResult(res);}catch(err){console.error('Spin result render error',err);DS().showToast('Hadiah spin berhasil didapat, tetapi tampilan hasil gagal ditampilkan.','error',4200);}
    }catch(err){
      console.error('Spin reward error',err);
      DS().showToast('Spin gagal diproses sebelum hadiah diterapkan.','error',3200);
    }finally{
      rolling=false;
...
/* v15 micro-interaction: restrained cockpit light, pointer only on capable devices. */
(function v15AmbientInteraction(){
  function boot(){
    var menu=document.getElementById('menuScreen');
    if(!menu || menu.__v15AmbientBound)return;
    menu.__v15AmbientBound=true;
    ...
    function paint(){
      ...
      menu.style.setProperty('--v15-mx',Math.max(0,Math.min(100,x)).toFixed(2)+'%');
      menu.style.setProperty('--v15-my',Math.max(0,Math.min(100,y)).toFixed(2)+'%');
    }
    menu.addEventListener('pointermove',function(ev){lastX=ev.clientX;lastY=ev.clientY;if(!raf)raf=requestAnimationFrame(paint);},{passive:true});
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();
})();


/* V18 — lightweight tactile feedback. One delegated listener, no per-button timers. */
(function v18TactileRipple(){
  function boot(){
    if(window.__v18RippleBound)return;
    window.__v18RippleBound=true;
    var layer=document.getElementById('indieFxLayer');
    if(!layer)return;
    document.addEventListener('pointerdown',function(ev){
      var el=ev.target&&ev.target.closest?ev.target.closest('button,.level-card,.shop-card,.ach-card,.challenge-row'):null;
      if(!el||el.disabled)return;
      var r=el.getBoundingClientRect();
      if(!r.width||!r.height)return;
      var dot=document.createElement('span');
      dot.className='v18-ripple'+(el.classList.contains('menu-main')||el.classList.contains('ov-btn')?' amber':'');
      var size=Math.max(44,Math.min(88,Math.max(r.width,r.height)*.7));
      dot.style.width=size+'px';dot.style.height=size+'px';
      dot.style.left=(ev.clientX-r.left)+'px';dot.style.top=(ev.clientY-r.top)+'px';
      dot.style.position='fixed';
      dot.style.left=ev.clientX+'px';dot.style.top=ev.clientY+'px';
      layer.appendChild(dot);
      setTimeout(function(){if(dot.parentNode)dot.parentNode.removeChild(dot);},480);
    },{passive:true});
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();
})();
